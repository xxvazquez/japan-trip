...
export function ActionRow({ icon, label, hint, onClick, disabled }: { icon?: IconName; label: string; hint?: string; onClick: () => void; disabled?: boolean }) {
  return (
    <li className={INSET_DIVIDER}>
      <button onClick={onClick} disabled={disabled} className="action w-full px-3.5 py-2.5 text-[0.9375rem] transition-colors duration-150 active:bg-ink/[0.07] active:opacity-100 disabled:opacity-50">
